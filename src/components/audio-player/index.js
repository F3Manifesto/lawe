...
const AudioPlayer = props => {
  const { autoPlay, file, loop } = props
  ...
  const load = () =>{
    ...
    if (audioRef.current.readyState >= 2) {
      setLoaded(true)
      console.log('audioRef.current.duration:', audioRef.current.duration)
      setDurationSeconds(parseInt(audioRef.current.duration))
      return setPlaying(autoPlay)
    }
  ...
  const mute = () => {
    if (muted()) {
      return setVolume(previousVolume)
    }
  ...
  const seek = (e) => {
    ...
    audioRef.current.currentTime = parseInt(audioRef.current.duration * seekPos);
  }

  const update = (e) => {
    setCurrentSeconds(parseInt(audioRef.current.currentTime))
  }
  ...
  return (
    <div className={styles.wrapper}>
			<div className={styles.playerControls}>
				<div id="stop">
					<a onClick={stop} title="Stop" href="">
          ...
          >
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
							{!playing && <path fill="currentColor" d="M15,10.001c0,0.299-0.305,0.514-0.305,0.514l-8.561,5.303C5.51,16.227,5,15.924,5,15.149V4.852c0-0.777,0.51-1.078,1.135-0.67l8.561,5.305C14.695,9.487,15,9.702,15,10.001z"/> }
							{
                playing && 
				...
				<div id="seek">
					<div className={styles.playerTimeline}>
						<div
              style={{
                width: `${parseInt(currentSeconds / durationSeconds * 100)}%`
              }} 
              className={styles.playerProgress}
            ></div>
						<div onClick={seek} className={styles.playerSeeker} title="Seek"></div>
					</div>
					<div className={styles.playerTime}>
						<div className={styles.playerTimeCurrent}>{convertTimeHHMMSS(currentSeconds)}</div>
						<div className={styles.playerTimeTotal}>{convertTimeHHMMSS(durationSeconds)}</div>
					</div>
				</div>
				{
          !showVolume &&
          <div id="download">
            <a onClick={download} href="#" title="Download">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
				...
				{/* {
          !showVolume && <div id="loop">
            <a onClick={setLooping(false)} href="#" title="Loop">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                {!looping && <path fill="currentColor" d="M1,12V5h3v6h10V8l5,4.5L14,17v-3H3C1.895,14,1,13.104,1,12z"/>}
                {
                  looping && <path fill="currentColor" d="M20,7v7c0,1.103-0.896,2-2,2H2c-1.104,0-2-0.897-2-2V7c0-1.104,0.896-2,2-2h7V3l4,3.5L9,10V8H3v5h14V8h-3V5h4C19.104,5,20,5.896,20,7z"/>
                }
              </svg>
            </a>
				  </div>
            ...
            title={`Volume (${volume}%)`} href="#">
						...
						{
              showVolume && 
                <input onChange={e => setVolume(e.target.value)} value={volume} className={styles.playerVolume} type="range" min="0" max="100" />
            }
					</a>
				</div>
			</div>
